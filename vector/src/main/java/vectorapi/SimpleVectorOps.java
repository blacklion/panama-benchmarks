...
public class SimpleVectorOps {
	...
	public static void main(String[] args) {
		...
		final VectorSpecies<Float> PFS2 = VectorSpecies.of(Float.TYPE, VectorShape.forBitSize(PFS.vectorBitSize() / 2));
		...
		FloatVector.fromArray(PFS, a, 0).rearrange(sh, im).intoArray(b, 0);
		System.out.println(Arrays.toString(b));

		FloatVector.fromArray(PFS, a, 0, new int[] {0, 0, 1, 1, 2, 2, 3, 3}, 0).intoArray(b, 0);
		System.out.println(Arrays.toString(b));
		...
		float sum = v.reduceLanes(VectorOperators.MUL, m);
		...
		FloatVector xx = FloatVector.fromArray(PFS2, a, 0).castShape(PFS, 0).reinterpretAsFloats().rearrange(SHUFFLE_RV_TO_CV_RE_ZERO);
		System.out.println(Arrays.toString(xx.toArray()));

		// And more
		FloatVector _10log10 = xx.abs().lanewise(VectorOperators.LOG10).mul(10.0f);
